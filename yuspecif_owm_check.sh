...
# Check alteration of the YUSPECIF file from OWM as compare to the benchmark
...
if [ $status -gt 1 ]; then
    ...
    echo "diff ./YUSPECIF ./YUSPECIF_owm > tmp_diff_YUSPECIF_owm_vs_benchmark.txt"
    echo `pwd`
    exit 1
fi

#compare length of diff
nl_ref=`wc -l < diff_YUSPECIF_owm_vs_benchmark.txt`
nl_bench=`wc -l < tmp_diff_YUSPECIF_owm_vs_benchmark.txt`

if [ "$nl_ref" -ne "$nl_bench" ]; then
    echo "! Warning : New differences with YUSPECIF_owm:" |tee -a tmp_mail.txt
    echo `pwd` |tee -a tmp_mail.txt
    echo "diff tmp_diff_YUSPECIF_owm_vs_benchmark.txt diff_YUSPECIF_owm_vs_benchmark.txt :"
    diff tmp_diff_YUSPECIF_owm_vs_benchmark.txt diff_YUSPECIF_owm_vs_benchmark.txt |tee -a tmp_mail.txt
    cat tmp_mail.txt | mail -s "COSMO - benchmark : changes against YUSPECIF_owm" "$recipients"
    rm tmp_mail.txt
fi
